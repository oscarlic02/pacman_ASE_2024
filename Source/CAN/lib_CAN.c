...
void handle_CAN_values(){
	if(pacman_status!=0){
	CAN_TxMsg.data[0] = (pacman_score & 0xFFDD) >> 8;
	CAN_TxMsg.data[1]=pacman_score & 0xFF;
	CAN_TxMsg.data[2] = pacman_lives & 0xFF;
	CAN_TxMsg.data[3] = current_time & 0xFF;
	CAN_TxMsg.len=4;
	CAN_TxMsg.id=2;
